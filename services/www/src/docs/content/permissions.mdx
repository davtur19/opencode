...
| `doom_loop`          | Tool name of the thrice-repeated identical call within one turn             |
...
`doom_loop` is checked when a turn repeats the same tool call: three
consecutive identical calls ask for `doom_loop` with the tool name as their
resource. The base policy asks, so the repeating call waits for approval; a
configured `deny` warns the model once, and only a repeat after that warning
stops the turn.

`lsp` is not a current V2 Core permission action.
